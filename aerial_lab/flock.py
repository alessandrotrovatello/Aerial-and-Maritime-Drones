...
class Flock():
    ...
    def get_laplacian_matrix(self):
        ...
        L = np.diag(np.sum(self._A, axis=1)) - self._A
        return L
    ...
    def compute_connectivity(self)->float:
        ...
        """
        rel_con = np.linalg.matrix_rank(self._L)/(self._nb_drones - 1)
        return rel_con
    ...
    def compute_deviation_energy(self)->float:
        ...
        for i in range(self._nb_drones):
            for j in range(self._nb_drones):
                if self._A[i,j] > 0:
                    energy += (np.linalg.norm(self._drones_pos[j] - self._drones_pos[i]) - PARAMS["d"])**2

        energy = 1/(nb_graph_edges + 1) * energy
        energy = energy/(PARAMS["d"]**2)
        return  energy
    ...
    def compute_velocity_mismatch(self)->float:
        ...
        """
        vel = 0.
        mean_vel = np.mean(self._drones_vel, axis=0)
        for i in range(self._nb_drones):
            vel += np.linalg.norm(self._drones_vel[i] - mean_vel)**2
        vel = vel/(2*self._nb_drones)
    ...
    def compute_cohesion_radius(self)->float:
        ...
        Returns:
            radius (float): 
                the cohesion radius
        """
        q_c = np.mean(self._drones_pos, axis=0)
        radius = max(np.linalg.norm(self._drones_pos - q_c, axis=1))
        return radius
