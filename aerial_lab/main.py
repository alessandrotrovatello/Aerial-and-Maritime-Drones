...
# Flocking parameters
NB_AGENTS = 50 # numbe of agents in the flock
...
INIT_AREA_FLOCK = np.array([30,30]) # agent are initialized inside a box of size (INIT_AREA_FLOCK[0] x INIT_AREA_FLOCK[1])
...
def main():
    ...
    while(True):
        ...
        if i in [0, int(5 / DT), int(10 / DT)]:
            fig1.savefig(f"flock_t_{round(i * dt, 1)}s.png")
        ...
        if round(i * dt, 1) == 10.0:
            fig2.savefig("values_t_5s.png")
